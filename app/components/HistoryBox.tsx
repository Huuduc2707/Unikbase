...
import { StyleProp, TextStyle, View, ViewStyle, Dimensions, ImageStyle, TouchableOpacity } from "react-native"
...
export const HistoryBox = observer(function HistoryBox(props: HistoryBoxProps) {
  ...
  const startAnimation = ()=>{
    ...
    animatedController.value = withTiming(!isDetailShown?-180:0, {duration: 200, easing: Easing.inOut(Easing.ease)})
  }
  ...
  const exiting = (values)=>{
    "worklet";
    const animations = {
      originY: withTiming(values.currentOriginY-10, {duration: 50, easing: Easing.inOut(Easing.ease)}),
  ...
  const layoutTransition = (values)=>{
    "worklet";
    const animations = {
      height: withTiming(values.targetHeight, {duration: 50, easing: Easing.inOut(Easing.ease)})
    };
    ...
    <Animated.View style={$styles} layout={layoutTransition}>
      <View style={$generalContainer}>
        <View style={$generalSubContainer}>
          ...
          <Text style={[$generalInfoText, $textMarginLeft]} text={`${data.date.toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: '2-digit'})} at ${data.date.getHours()%12<10&&data.date.getHours()%12!==0?0:""}${data.date.toLocaleTimeString('en-US', {hour: '2-digit', minute: '2-digit', hour12: true})}`}
          />
        </View>
        {
          !hideStatus &&
          <Text style={[$statusText, {color: statusTextColor[data.status]}]} tx={status[data.status]}/>
        }
      </View>
      <View style={[$generalContainer, {marginTop: 10}]}>
        <View style={$generalSubContainer}>
          <Icon icon={downloadOperator.includes(operator[data.operator])?"download":"upload"} style={$mainIcon}/>
        ...
        <TouchableOpacity style={{justifyContent: 'flex-end'}}activeOpacity={0.7} onPress={startAnimation}>
      ...
      </View>
      {
        ...
        <Animated.View style={$detailedInfoContainer} entering={entering} exiting={exiting}>
          <Line style={$line}/>
          <Text style={$detailedHeader} tx={"mainpageNavigator.history.tokenHistory.detailHeader.digitalTwinName"}/>
          <Text style={$detailedInfoText} text={data.tokenName}/>
          <Line style={$line}/>
          <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
            <View>
              <Text style={$detailedHeader} tx={"mainpageNavigator.history.tokenHistory.detailHeader.status"}/>
              <Text style={$detailedInfoText} tx={status[data.status]}/>              
            </View>
            <View style={{alignItems: 'flex-end'}}>
              <Text style={$detailedHeader} tx={"mainpageNavigator.history.tokenHistory.detailHeader.date"}/>
              <Text style={$detailedInfoText} text={`${data.date.toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: '2-digit'})} at ${data.date.getHours()%12<10&&data.date.getHours()%12!==0?0:""}${data.date.toLocaleTimeString("en-US", {hour: '2-digit', minute: '2-digit', hour12: true})}`}/>
            </View>
          </View>
          <Line style={$line}/>
          <Text style={$detailedHeader} tx={"mainpageNavigator.history.tokenHistory.detailHeader.from"}/>
          <Text style={$detailedInfoText} text={data.senderID}/>
          <Line style={$line}/>
          <Text style={$detailedHeader} tx={"mainpageNavigator.history.tokenHistory.detailHeader.to"}/>
          <Text style={$detailedInfoText} text={data.receiverID}/>
          <View style={$costSummaryContainer}>
            <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
              <Text style={$costSummaryText} tx={"mainpageNavigator.history.tokenHistory.detailHeader.amount"}/>
              <Text style={$costSummaryText} text={`$${data.amount.toFixed(2)}`}/>
            </View>
            <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
              <Text style={$costSummaryText} tx={"mainpageNavigator.history.tokenHistory.detailHeader.transactionFee"}/>
              <Text style={$costSummaryText} text={`$${data.transactionFee===0?0:""}${data.transactionFee.toFixed(2)}`}/>
            </View>
            <Line style={$line}/>
            <View style={{flexDirection: 'row', justifyContent: 'space-between', marginTop: 13}}>
              <Text style={$costSummaryText} tx={"mainpageNavigator.history.tokenHistory.detailHeader.totalAmount"}/>
              <Text style={$costSummaryText} text={`$${(data.amount - data.transactionFee).toFixed(2)}`}/>
            </View>
          </View>
        </Animated.View>
      }
