...
interface DigitalTwinScreenProps
  extends NativeStackScreenProps<AppStackScreenProps<"DigitalTwin">> {}
...
export const DigitalTwinScreen: FC<DigitalTwinScreenProps> = observer(function DigitalTwinScreen() {
  ...
  const documentList: DocumentList[] = [
    ...
    },
    {
      ...
      document: [{ name: "Object-certificate.doc" }],
    },
    {
    ...
    },
    {
      ...
      document: [{ name: "Invoice-for-artist.pdf" }],
    },
  ]
  ...
  return (
    <Screen style={$root} preset="fixed">
      ...
      <View style={$headerContainer}>
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={() => navigation.navigate("MainPage", { screen: "Wallet" })}
        >
      ...
      >
        ...
        <FieldHeaderBar
          ...
          fieldContent={[
            ...
            },
            {
              ...
              content:
                "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur sit amet pretium elit, non pellentesque magna. Aenean mollis tortor lorem, quis volutpat dolor aliquet condimentum. Nunc dignissim commodo sem vel scelerisque.",
            },
          ]}
        ...
        <FieldHeaderBar
          headerTextStyle={$boldText}
          tx={"mainpageNavigator.tabName.specification"}
          destinationSreen="Specification"
        />
        <View style={$specificationSection}>
          ...
          <View>
            {specificationTag.map((value, index) => (
              <Text
                style={$specificationText}
                key={(index + 1) * 6}
                text={value.content === "" ? "unknown" : value.content}
              />
            ))}
          </View>
        </View>
        ...
        <View style={$sizeSection}>
          ...
          <View style={$sizeSectionSubContainer}>
            {[0, 1].map((_, index) => (
              <View
                key={index}
                style={[$sizeInfoContainer, { borderRightWidth: index === 0 ? 1 : 0 }]}
              >
                  ...
                  if (idx % 2 === index % 2)
                    return (
                      <View key={(idx + 1) * 0.1} style={$sizeInfoSubContainer}>
                        ...
                        <View
                          style={[
                            $sizeInfoSubSubContainer,
                            { borderBottomWidth: idx < sizeInfo.length / 2 ? 1 : 0 },
                          ]}
                        >
                          <Text style={[$labelText, $sizeInfoText]} tx={size.type} />
                          <Text
                            style={$sizeInfoText}
                            text={size.value === "" ? "unknown" : size.value}
                          />
                        </View>
                      </View>
                    )
        ...
        <FieldHeaderBar
          headerTextStyle={$boldText}
          tx={"mainpageNavigator.tabName.gallery"}
          destinationSreen="Gallery"
        />
        <View style={$gallerySection}>
          <Carousel
            ...
            swipeThreshold={10}
            onSnapToItem={(index) => {
              return setActiveSlideIndex(index)
            }}
          />
          <Pagination
        ...
        <FieldHeaderBar
          headerTextStyle={$boldText}
          tx={"mainpageNavigator.tabName.document"}
          destinationSreen="Document"
        />
      ...
      <VersatileModal
        visibility={isCreateTokenModalVisible}
        setVisibility={setIsCreateTokenModalVisible}
      />
    </Screen>
  )
})
